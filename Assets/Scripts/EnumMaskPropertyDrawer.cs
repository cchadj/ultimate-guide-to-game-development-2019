...
public class EnumMaskPropertyDrawer : PropertyDrawer
{
    ...
    {
        return Convert.ChangeType(_enumValues.GetValue(_index), _enumUnderlyingType);
    }
    ...
    void ToggleIndex(int _index, bool _set)
    {
        if(_set)
        {
            ...
            //enum = enum | val
            _theEnum = DoOrOperator(_theEnum, GetEnumValue(_index), _enumUnderlyingType);
        }
        else
        {
            ...
            //enum = enum & ~val
            _theEnum = DoAndOperator(_theEnum, notVal, _enumUnderlyingType);
        }
    ...
    {
        object val = DoAndOperator(_theEnum, GetEnumValue(_index), _enumUnderlyingType);
    ...
    {
        return GetEnumValue(_index).Equals(Convert.ChangeType(0, _enumUnderlyingType));
    }
